...
public class MobileActions extends CommonOps
{
    ...
    @Step("Split Balance line and return number of $")
    public static double splitBalance(MobileElement elem)
    {
        wait.until(ExpectedConditions.visibilityOf(elem));
        String str = elem.getText().substring(elem.getText().lastIndexOf(" ")+1);
